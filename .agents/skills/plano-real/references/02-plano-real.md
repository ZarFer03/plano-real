...
## Objetivo
Representar por dónde camina el trabajo, sus desvíos y esperas, con evidencia de los casos y sin dibujar un proceso ideal.
## Qué necesita para empezar
Leer [las convenciones](00-convenciones.md). Alcance y permisos vigentes, casos reales elegidos y al menos uno caminado de punta a punta, con acceso a entrevistas y registros.
## Pasos de trabajo
1. Recorrer los casos y variantes con los criterios de abajo. **Termina cuando:** cada recorrido tiene identidad, fecha, participantes y pendientes explícitos.
2. Capturar nodos, rutas y excepciones; calcular los contadores con método y fuente. **Termina cuando:** el flujo está trazado y se distingue lo observado de lo declarado.
3. Generar las vistas desde las tablas y revisar vigencia y legibilidad. **Termina cuando:** el diagrama corresponde a la fuente y no oculta ramas ni evidencia pendiente.
...
## Si falta evidencia
Si falta una variante, documentar que no se encontró en la ventana. Si un paso solo fue declarado, conservar su etiqueta y pendiente aparte; no presentarlo como observado ni cambiarlo. Si no se pueden calcular los contadores, el plano sigue siendo borrador.
...
Entregar a fase 3 el plano vigente, registros de casos, contadores con método y excepciones con estado de conteo. Identificar el tramo que puede medirse y qué pendiente limita su uso.
